...
let findSeatByCoordinates rowNumber seatNumber =
    ...
    let sql =
        "SELECT SeatId, RowNumber, SeatNumber, IsReserved FROM Seats WHERE RowNumber = @row AND SeatNumber = @seat"
...
let saveTicket (ticket: Ticket) =
    ...
    try
        let sql =
            "INSERT INTO Tickets (TicketId, SeatId, UserId) VALUES (@ticketId, @seatId, @userId)"
