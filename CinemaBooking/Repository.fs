...
let saveUser (user: User) =
    ...
    try
        ...
        Some user
    with :? SqliteException as ex when ex.Message.Contains("UNIQUE") ->
        None
...
let findUserByUsername username =
    ...
    let parameters = dict [ "Username", box username ]
...
let createRoom noRows noCol =
    ...
    for row in 1..noRows do
        for col in 1..noCol do
            let seatSql =
                "INSERT INTO seats (room_id, row_number, seat_number, is_reserved) VALUES (@RoomId, @Row, @Col, 0)"
...
let getSeatsForScreening screeningId =
    ...
    | Some screening ->
        ...
        let parameters =
            dict [ "ScreeningId", box screeningId; "RoomId", box screening.RoomId ]
...
let saveTicket (ticket: Ticket) =
    ...
    let parameters =
        dict
            [ "SeatId", box ticket.SeatId
              "ScreeningId", box ticket.ScreeningId
              "UserId", box ticket.UserId
              "CreatedAt", box (ticket.CreatedAt.ToString("yyyy-MM-dd HH:mm:ss")) ]

    try
        let ticketId = connection.ExecuteScalar<int64>(sql, parameters) |> int
        Some { ticket with TicketId = ticketId }
    with _ ->
        None
