...
let seedSeatsIfEmpty (rows: int) (cols: int) =
    ...
    if count = 0 then
        // Insert all seats
        let insertSql =
            "INSERT INTO Seats (RowNumber, SeatNumber, IsReserved) VALUES (@row, @seat, 0)"
...
let getSeatsFromDb () : Seat list = getAllSeats ()
...
let getSeatLayout () : Seat list list = getSeatsFromDb () |> getSeatGrid
...
let findSeatInDb row seatNumber = findSeatByCoordinates row seatNumber
