...
let main argv =
    ...
    | None -> printfn "Failed to save user"
    ...
    | Some user -> printfn "User found: %s" user.Username
    | None -> printfn "User not found"
    ...
    | Some seat -> printfn "Seat found: ID=%d" seat.SeatId
    | None -> printfn "Seat not found"
    ...
    | Some seat ->
        ...
        | Some reservedSeatId ->
            ...
            match findSeatByCoordinates 5 7 with
            | Some updatedSeat -> printfn "  -Verification: Seat is now reserved = %b" updatedSeat.IsReserved
            | None -> ()
        | None -> printfn "Failed to reserve seat"
    | None -> printfn "Seat not found"
    ...
    | Some seat ->
        match saveSeatReservation seat.SeatId with
        | Some _ -> printfn "Seat reserved successfully!"
        | None -> printfn "Failed to reserve seat"
    | None -> printfn "Seat not found"
    ...
    | Some seat ->
        match saveSeatReservation seat.SeatId with
        | Some _ -> printfn "Seat reserved"
        | None -> printfn "Cannot reserve seat already reserved"
    | None -> printfn "Seat not found"
    ...
    | Some user ->
        ...
        | Some seat ->
            ...
            | None -> printfn "Failed to create ticket"
        | None -> printfn "Seat not found"
    | None -> printfn "User not found"
    ...
    reservedSeats
    |> List.iter (fun s -> printfn "  Seat ID: %d | Row: %d, Seat: %d" s.SeatId s.RowNumber s.SeatNumber)
